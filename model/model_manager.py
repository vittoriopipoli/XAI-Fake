...
class ModelManager():
    ...
    def train(self, train_dataloader, eval_dataloader, checkpoint="example/nn_model", DEVICE="cuda"):
        cudnn.benchmark 
        running_corrects = 0
        trainLoss = []
        validationLoss = []
        ...
        for epoch in range(self.config.trainer.epochs):
            if STOP:
                ...
                break
            logging.info('\nStarting epoch {}/{}, LR = {}'.format(epoch+1, self.config.trainer.epochs, self.scheduler.get_last_lr()))
            tloss = []
            running_corrects = 0
            running_elements = 0
            ...
            if vloss < lowest_val_loss:
                ...
                logging.info("############################################ New lowest_val_loss reached #########################")                 
